...
class PolizaDeIngresoVariosService extends ProcesadorService{
        ...
        def procesar(Poliza poliza){
            ...
            def ingresos = MovimientoDeCuenta.findAll('from MovimientoDeCuenta m where date(m.fecha) = ? and m.concepto = ? ',[dia,'SALDO_DEUDOR'])
            ingresos.each { i ->
                log.info 'Procesando ingreso de tipo SALDO_DEUDOR por : '+i
                procesarDevoucionesVarias(poliza,i)
            }
        	cuadrar(poliza)
    		depurar(poliza)
    		save poliza
        ...
        def procesarDevoucionesVarias(def poliza, def ingreso){
            def dia = poliza.fecha
            def asiento='DEVOLUCION PROVEEDOR'
            def desc = CuentaContable.findByClave(ingreso.comentario)?.descripcion
                ...
                poliza.addToPartidas(
                    ...
                    haber:0.0,
                    asiento:asiento,
                    descripcion:desc?:'NO REGISTRO CUENTA CONTABLE ',
                    referencia:"$ingreso.referenciaBancaria",
                    ,fecha:poliza.fecha
                    ,tipo:poliza.tipo
